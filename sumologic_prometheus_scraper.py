...
class SumoPrometheusScraper:
    ...
    def run(self):
        ...
        log.info(
            "target={0}  will send {1} metrics to sumo".format(self.name, len(metrics))
        )
        batches = list(
            self.chunk_metrics(metrics_list=metrics, batch_size=self.batch_size)
        )
        log.debug("target={0}  pushing to sumo with headers: {1}".format(self.name))
        event_loop = asyncio.new_event_loop()
        event_loop.run_until_complete(
            self._post_to_sumo(batches=batches, headers=headers, target_name=self.name)
        )
    ...
    def scrape_metrics(self):
        ...
        try:
            ...
            resp = self._requests_retry_session().get(
                ...
                verify=target_config.get("verify", None),
                headers=headers,
            )
            if resp.status_code != 200:
                log.error(
                    "received status code {0} from target {1}: {2}".format(
            ...
            log.error(
                "unable to scrape metrics from target {0}: {1}".format(self.name, e)
            )
        ...
        for metric_string in prometheus_metrics:
            for family in text_string_to_metric_families(metric_string):
                for sample in family.samples:
                    ...
                    if (
                        metric_data[2].lower() == "nan"
                    ):  # carbon2 format cannot accept NaN values
                        continue
                    carbon2_metric = "metric={0} ".format(metric_data[0])
                    for attr, value in ast.literal_eval(metric_data[1]).items():
                        if value == "":
                            # carbon2 format cannot accept empty values
                            value = "none"
                        if " " in value:
                            # carbon2 format cannot accept values with spaces
                    ...
                    if len(target_config.get("include_metrics", [])) != 0:
                        if metric_data[0] in target_config["include_metrics"]:
                            if metric_data[0] not in target_config.get(
                                "exclude_metrics", []
                            ):
    ...
    async def _post_to_sumo(self, batches, headers, target_name):
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=self.post_threads
        ) as executor:
            event_loop = asyncio.get_event_loop()
            futures = [
                event_loop.run_in_executor(
                    executor, self._compress_and_send, idx, batch, headers, target_name
                )
    ...
    def _compress_and_send(self, idx, batch, headers, target_name):
        ...
        log.debug(
            "target={0} batch={1}  size before compression: {2} mb".format(
                target_name, idx, raw_size
            )
        )
        compressed_batch = gzip.compress(
            data=batch.encode(encoding="utf-8"), compresslevel=1
        )
        compressed_size = sys.getsizeof(compressed_batch) / 1024 / 1024
        log.debug(
            "target={0} batch={1} size after compression: {2} mb".format(
                target_name, idx, compressed_size
            )
        )
        ...
        if resp.status_code != 200:
            log.error(
                "target={0} batch={1}  error sending batch to sumo: {2}".format(
                    target_name, idx, resp.content
                )
            )
        else:
            log.info(
                "target={0} batch={1}  successfully posted batch to sumo".format(
                    target_name, idx
                )
            )
...
target_config_schema = global_config_schema.extend(
    {Required("url"): Url(), Required("name"): str}
)
